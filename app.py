...
async def add_blacklist_word(word: str):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("INSERT OR IGNORE INTO blacklist(word) VALUES(?)", (word.lower(),))
        await db.commit()

async def get_blacklist() -> set[str]:
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute("SELECT word FROM blacklist") as cur:
...
async def ai_extract_city(text: str) -> str:
    """
    Matndan AQSh shahar / shtat nomini ajratadi.
    Reply format: "City, State, USA" yoki "City" yoki ""
    """
    ...
    try:
        r = await ai.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Extract only the first US city or state name from the user message. "
                            "Reply in format 'City, State, USA' or just 'City' if state unknown. "
                            "If nothing found, reply empty. Never explain."},
                {"role": "user", "content": t}
            ],
            temperature=0
        )
...
def parse_gmaps_link(url: str) -> tuple[float | None, float | None]:
    try:
        ...
        # 2) ?q=lat,lng
        if m := re.search(r'[?&]q=(-?\d+(?:\.\d+)?),([+-]?\d+(?:\.\d+)?)', url):
            return float(m.group(1)), float(m.group(2))
        ...
        # 4) /search/lat, +lng (butun son va float)
        if m := re.search(r'/search/(-?\d+(?:\.\d+)?)\s*,\s*[+]?\s*(-?\d+(?:\.\d+)?)', url):
            return float(m.group(1)), float(m.group(2))
...
async def save_edit_location_link(message: types.Message, state: FSMContext):
    ...
    # yangi koordinatalarni ham olamiz
    lat, lng = parse_gmaps_link(new_link)
    if lat is None:
        await message.answer(
            "❌ Havola yaroqsiz! Qayta yuboring:\n"
            "Masalan: https://maps.app.goo.gl/abcd   yoki  https://www.google.com/maps/ …"
        )
        return
    ...
    for key in ('text_user', 'text_channel'):
        if key not in PLACES[idx]:
            continue
        old = PLACES[idx][key]
        new = re.sub(r'(📍 <a\s+href\s*=\s*["\'])[^"\']*(["\'][^>]*>[^<]*</a>)',
                    rf'\g<1>{new_link}\g<2>', old, flags=re.I)
        PLACES[idx][key] = new
...
async def got_second_number(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "UPDATE places SET name=?, text_user=?, text_channel=? WHERE id=?",
...
async def by_text(message: types.Message):
    ...
    # ---------- 1) OpenAI bilan shahar nomini ajratamiz ----------
    city_name = await ai_extract_city(raw)

    if not city_name:
        return

    # ---------- 2) koordinatalarni topish ----------
    lat, lng = await coords_from_any(city_name)
    if lat is None:
        return

    # ---------- 3) 100 km radiusda restoranlar ----------
